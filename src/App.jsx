    ...
    <div className="min-h-screen bg-emerald-50 p-4 md:p-8 font-sans text-slate-800">
      <header className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-emerald-700">Manage your applications</p>
        </div>
      ...
      <div className="bg-white rounded-3xl shadow-lg border border-emerald-100 overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-emerald-50/50">
            <tr className="text-emerald-900 font-bold uppercase text-[10px] tracking-widest">
              <th className="p-6">Position</th><th className="p-6">Salary (Editable)</th><th className="p-6">Logistics</th><th className="p-6">Status</th><th className="p-6 text-right">Action</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-emerald-50">
            {items.map((val) => {
              const rowStyle = val.status === 'applied' ? 'bg-emerald-50/30' : val.status === 'offer' ? 'bg-emerald-100/30' : 'bg-white';
              return (
                <tr key={val.id} className={`${rowStyle} hover:bg-emerald-100/50 transition-all`}>
                  ...
                  <td className="p-6 text-xs font-semibold">{val.deadline}</td>
                  <td className="p-6">
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-emerald-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <form onSubmit={handleCreateEntry} className="bg-white p-8 rounded-3xl w-full max-w-lg shadow-2xl relative">
            ...
            <h3 className="text-2xl font-bold mb-6">New Position</h3>
            <div className="grid grid-cols-2 gap-4">
              ...
              <input className="p-3 border rounded-xl" placeholder="Pay" onChange={e => setFormData({...formData, gehalt: e.target.value})} />
              <input className="p-3 border rounded-xl" placeholder="Future Salary" onChange={e => setFormData({...formData, earnings: e.target.value})} />
            </div>
            <button type="submit" className="w-full mt-6 bg-emerald-700 text-white py-4 rounded-2xl font-bold hover:bg-emerald-800">Add to Pipeline</button>
          </form>
        </div>
      )}
    </div>
