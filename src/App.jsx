    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-900">
        <div className="bg-white p-8 rounded-xl shadow-2xl w-full max-w-sm text-center">
          ...
          <button onClick={() => { if(password === "9843543456") setIsAuthenticated(true); }} className="w-full bg-indigo-600 text-white p-2 rounded font-bold">Unlock</button>
        </div>
      </div>
    );
  }
      ...
      <table className="w-full bg-white border shadow-sm rounded-xl overflow-hidden">
        ...
        <tbody className="divide-y text-xs">
          {items.map((val) => {
            ...
            if (val.status === 'applied') color = "bg-blue-50 text-blue-700 border-blue-200";
            if (val.status === 'interview') color = "bg-amber-50 text-amber-700 border-amber-200";
            if (val.status === 'offer') color = "bg-emerald-50 text-emerald-700 border-emerald-200";
            if (val.status === 'rejected') color = "bg-rose-50 text-rose-700 border-rose-200";
            ...
            return (
              <tr key={val.id}>
                ...
                <td className="p-4">
                  <select value={val.status} onChange={(e) => handleStatusChange(val.id, e.target.value)} className={`p-1 border rounded font-bold ${color}`}>
